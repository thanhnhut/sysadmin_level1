...
Trình duyệt web, máy chủ và các ứng dụng web có liên quan tất cả đều giao tiếp với nhau thông qua HTTP (Hypertext Transfer Protocol). HTTP là ngôn ngữ phổ biến của Internet toàn cầu hiện đại.

Chương này là một tổng quan ngắn gọn của HTTP. Bạn sẽ thấy cách ứng dụng web sử dụng HTTP để giao tiếp, và bạn sẽ có được một hình dung về cách thức HTTP làm công việc của mình. Đặc biệt, chúng ta tìm hiểu  về:
...
Chúng tôi đã có rất nhiều điều để tìm hiểu, do đó, hãy bắt đầu chuyến tham quan của chúng ta về HTTP.

<a name="11"></a>
###1.1 Đa phương tiện chuyển phát nhanh của Internet

Hàng tỷ hình ảnh JPEG, các trang HTML, các file văn bản, phim MPEG, file âm thanh WAV, Java applet, và nhiều tiến trình qua Internet mỗi ngày. HTTP di chuyển số lượng lớn các thông tin này một cách nhanh chóng, thuận tiện, và đáng tin cậy từ các máy chủ web trên toàn thế giới với các trình duyệt web trên máy tính của mọi người.

Bởi vì HTTP sử dụng giao thức truyền dữ liệu đáng tin cậy, nó đảm bảo rằng dữ liệu của bạn sẽ không hư hỏng hoặc bị xáo trộn trong quá trình chuyển, ngay cả khi nó đến từ phía bên kia của địa cầu. Điều này tốt cho bạn bởi vì bạn có thể truy cập thông tin mà không cần lo lắng về tính toàn vẹn của nó. Đường truyền đáng tin cậy rất tốt cho bạn nó giúp bạn  như là một nhà phát triển ứng dụng Internet, bởi vì bạn không cần phải lo lắng về thông tin liên lạc HTTP bị phá hủy, sao chép, hoặc bị bóp méo quá trình. Bạn có thể tập trung về lập trình các chi tiết khác biệt của ứng dụng của bạn, mà không lo lắng về những sai sót và nhược điểm của Internet.
...
Bạn có thể sử dụng HTTP khách mỗi ngày. Các HTTP khách phổ biến nhất là một trình duyệt web, chẳng hạn như Microsoft Internet Explorer hoặc Netscape Navigator. Trình duyệt Web yêu cầu đối tượng HTTP từ máy chủ và hiển thị các đối tượng trên màn hình của bạn. 

<a name="13"></a>
###1.3 Tài nguyên

Các web server lưu trữ tài nguyên web. Một nguồn tài nguyên web là nguồn gốc của nội dung web. Các loại nguồn tài nguyên web đơn giản nhất là một tập tin tĩnh về hệ thống tập tin máy chủ web. Những tập tin này có thể chứa bất cứ điều gì: chúng có thể là các file văn bản, file HTML, các file Microsoft Word, Adobe Acrobat, tập tin hình ảnh JPEG, AVI file phim, hoặc bất kỳ định dạng khác mà bạn có thể nghĩ đến.
...
Tóm lại, một nguồn tài nguyên là bất kỳ loại nguồn nội dung. Một tập tin có chứa dự báo bán hàng bảng tính của công ty bạn là một nguồn tài nguyên, một công cụ tìm kiếm Internet là một nguồn tài nguyên,...
...
Có hàng trăm loại MIME phổ biến, nhiều loại được thử nghiệm nhiều hơn hoặc hạn chế sử dụng. Rất nhiều danh sách kiểu MIME được cung cấp trong Phụ lục D.
...
Hình 1-4 cho thấy cách các URI chỉ định các giao thức HTTP để truy cập tài nguyên lưỡi cưa GIF trên máy chủ lưu trữ của Joe. Với URI, HTTP có thể lấy các đối tượng. URIs đến trong hai hướng, được gọi là URL và URNs. Chúng ta hãy nhìn qua mỗi một trong các loại định danh tài nguyên bây giờ.
...
Các bộ định vị tài nguyên thống nhất (URL) là hình thức phổ biến nhất của định danh tài nguyên. URL mô tả vị trí cụ thể của một tài nguyên trên một máy chủ cụ thể. Chúng nói với bạn một cách chính xác làm thế nào để lấy tài nguyên từ một máy chủ, vị trí cố định chính xác. Hình 1-4 cho thấy cách một URL nói chính xác nơi một nguồn tài nguyên nằm và làm thế nào để truy cập vào nó. Bảng 1.1 cho thấy một vài ví dụ về URL.
...
Hương thứ hai của URI là tên tài nguyên thống nhất, hoặc URN. Một URN phục vụ như là một tên duy nhất cho một phần của nội dung, độc lập với các nơi có tài nguyên hiện đang cư trú. Những vị trí URNs độc lập  cho phép các nguồn lực để di chuyển từ nơi này đến nơi khác. URNs cũng cho phép các nguồn lực để được truy cập bởi nhiều giao thức truy cập mạng trong khi duy trì cùng tên. Ví dụ, URN sau đây có thể được sử dụng để đặt tên cho tài liệu tiêu chuẩn Internet "RFC 2141" không phân biệt nơi nó được lưu trữ (thậm chí nó có thể được sao chép ở một vài nơi):

**urn:ietf:rfc:2141**

URNs vẫn còn được thử nghiệm và chưa được áp dụng rộng rãi. Để làm việc hiệu quả, URNs cần có một cơ sở hạ tầng hỗ trợ giải quyết vị trí tài nguyên; việc thiếu một cơ sở hạ tầng như vậy cũng đã chậm lại tiến trình. Nhưng URNs có một số hứa hẹn thú vj cho tương lai. Chúng ta sẽ thảo luận về URNs trong một vài phần trong Chương 2, nhưng hầu hết các phần còn lại của cuốn sách này tập trung chủ yếu vào các URLs. 
